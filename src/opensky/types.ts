...
export interface IAircraftRoute {
  icao24: string;
...
export interface IAircraftMetadata {
  icao24: string;
...
export interface IAircraftTrack {
  ...
  route?: IAircraftRoute;
  metadata?: IAircraftMetadata;
  flightDetails?: IAircraftFlightDetails;
};

export interface IAircraftFlightDetails {
  airline?: string;
  airlineIcao?: string;
  airlineIata?: string;
  flightNumber?: string;
...
export type FlightStatus = 'scheduled' | 'boarding' | 'departed' | 'en_route' | 'approaching' | 'landed' | 'cancelled' | 'diverted' | 'unknown';

export interface IFlightTimelineEvent {
  phase: string;
  label: string;
  time?: number | null;
  completed: boolean;
...
export interface IFlightStatistics {
...
export interface IAltitudeDataPoint {
  time: number;
  altitude: number;
}

export interface ISpeedDataPoint {
  time: number;
  speed: number;
}
...
export interface IAirportData {
  ...
  source?: string;
  runways?: IRunwayData[];
  frequencies?: IFrequencyData[];
}

export interface IRunwayData {
  id: string;
  length_ft: number;
  width_ft: number;
  surface: string;
